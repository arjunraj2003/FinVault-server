...
async function runTests() {
    try {
        ...
        console.log('\n--- Creating an Account ---');
        const accRes = await axios.post(`${API_URL}/account`, {
        ...
        const createRes = await axios.post(`${API_URL}/budget`, budgetPayload, config);
        ...
        const getRes = await axios.get(`${API_URL}/budget`, config);
        console.log(`✓ Retrieved ${getRes.data.data.length} budget(s)`);

        // 4. Test Get Budget By ID
        console.log('\n--- Testing: Get Budget By ID ---');
        const getByIdRes = await axios.get(`${API_URL}/budget/${budgetId}`, config);
        ...
        const updateRes = await axios.put(`${API_URL}/budget/${budgetId}`, { amount: 600 }, config);
        ...
        // Verify Progress
        const progressRes = await axios.get(`${API_URL}/budget/progress/${budgetId}`, config);
        ...
        await axios.delete(`${API_URL}/budget/${budgetId}`, config);
        console.log(`✓ Deleted budget ${budgetId}`);

        // Verify Deletion
        try {
            await axios.get(`${API_URL}/budget/${budgetId}`, config);
